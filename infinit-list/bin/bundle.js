...
/***/ function(module, exports, __webpack_require__) {
	...
	RowControler = __webpack_require__(1);

	document.addEventListener('DOMContentLoaded', function() {
	  var BUFFER_LIST_LENGTH, SHORT_LIST_LENGTH, body, consistencyResult$, container, initVars, onMouseUp, previousHeight, rowControler, testOptions, viewportElement;
	  body = document.querySelector('body');
	  body.innerHTML = __webpack_require__(44)();
	   ...
	   */
	  viewportElement = document.querySelector('.longListViewport');
	  ...
	  document.querySelector('.initBtn').addEventListener('click', function() {
	    ...
	    return rowControler.init(nbRowsToIni);
	  });
	  document.querySelector('.nbRowsToIni').addEventListener('keyup', function(e) {
	    var nbRowsToIni;
	    if (e.keyCode === 13) {
	  ...
	  document.querySelector('.goDownBtn').addEventListener('click', function() {
	    return rowControler.goDownHalfBuffer(0.5);
	  });
	  document.querySelector('.goUpBtn').addEventListener('click', function() {
	    return rowControler.goUpHalfBuffer(0.5);
	  });
	  document.querySelector('.addBtn').addEventListener('click', function() {
	    return rowControler.addRowAfterSelected();
	  });
	  document.querySelector('.addAtRkBtn').addEventListener('click', function() {
	    var rk;
	    rk = parseInt(document.querySelector('.insertionRk').value);
	    return rowControler.addRowAtRk(rk);
	  });
	  document.querySelector('.deleteBtn').addEventListener('click', function() {
	    return rowControler.deleteSelected();
	  });
	  document.querySelector('.deleteAllBtn').addEventListener('click', function() {
	    return rowControler.deleteAll();
	  });
	  document.querySelector('.unActivateBtn').addEventListener('click', function() {
	    return rowControler.unActivateScrollListener();
	  });
	  document.querySelector('.activateBtn').addEventListener('click', function() {
	    return rowControler.activateScrollListener();
	  });
	  consistencyResult$ = document.querySelector('.consistencyResult');
	  document.querySelector('.testLongListBtn').addEventListener('click', function() {
	    var err, error, isOk;
	    isOk = true;
	    try {
	    ...
	    if (isOk) {
	      ...
	      return consistencyResult$.classList.remove('alert');
	    } else {
	      consistencyResult$.textContent = 'Consistency NOK';
	      return consistencyResult$.classList.add('alert');
	    }
	  });
	  previousHeight = null;
	  onMouseUp = function() {
	    ...
	    if (newHeight !== previousHeight) {
	      rowControler.resize();
	      return previousHeight = newHeight;
	    }
	  };
	  onMouseUp();
	  document.querySelector('#resizable-container').addEventListener('mouseup', function(e) {
	    return onMouseUp();
	  });
	  ...
	  initVars = function() {
	    ...
	    return console.log('SHORT_LIST_LENGTH', SHORT_LIST_LENGTH);
	  };
	  initVars();
	  testOptions = {
	  ...
	  mocha.setup('bdd');
	  __webpack_require__(47)(testOptions);
	  return mocha.run();
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var LinkedList, LongListRows, RowControler, expect;

	LongListRows = __webpack_require__(2);

	LinkedList = __webpack_require__(3);

	expect = __webpack_require__(4).expect;

	module.exports = RowControler = (function() {
	  function RowControler(viewportElement) {
	     ...
	     */
	    var checkIfSelected, onRowClick, options;
	    options = {
	      DIMENSIONS_UNIT: 'em',
	      ROW_HEIGHT: 2,
	      BUFFER_COEF: 3,
	      ...
	      THROTTLE: 20,
	      MAX_SPEED: 1.5,
	      onRowsCreatedCB: (function(_this) {
	        return function(rowsToCreate) {
	          var i, len, row, rowTxt, rowsList;
	          rowsList = _this.rowsList;
	          for (i = 0, len = rowsToCreate.length; i < len; i++) {
	            row = rowsToCreate[i];
	      ...
	      onRowsMovedCB: (function(_this) {
	        return function(rowsToDecorate) {
	          var components, i, len, row, rowTxt, rowsList;
	          rowsList = _this.rowsList;
	          for (i = 0, len = rowsToDecorate.length; i < len; i++) {
	            row = rowsToDecorate[i];
	            rowTxt = rowsList.at(row.rank).data;
	     ...
	     */
	    this.longList = new LongListRows(viewportElement, options);
	    this._test = this.longList._test;

	    /** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	     * listeners of rows clicks
	     */
	    this.selectedRows = {};
	    onRowClick = (function(_this) {
	      return function(e) {
	        var row$;
	        ...
	        return _this.toggleRow$(row$);
	      };
	    })(this);
	    this.toggleRow$ = function(row$) {
	      var row, rowID;
	      row = this.rowsList.at(row$.dataset.rank);
	      rowID = row.id;
	      if (this.selectedRows[rowID]) {
	        ...
	        return this.selectedRows[rowID] = row;
	      }
	    };
	    checkIfSelected = (function(_this) {
	      return function(row$) {
	        var rowID;
	        rowID = _this.rowsList.at(row$.dataset.rank).id;
	  ...
	  RowControler.prototype.init = function(nbRowsToIni) {
	    this.selectedRows = {};
	    this._initRowsList(nbRowsToIni);
	    return this.longList.initRows(nbRowsToIni);
	  };

	  RowControler.prototype._initRowsList = function(nRows) {
	    var i, ref, rk, row, rowsList;
	    rowsList = new LinkedList();
	    for (rk = i = 0, ref = nRows - 1; i <= ref; rk = i += 1) {
	      row = rowsList.append('');
	      row.data = "row: id_" + row.id;
	    }
	    return this.rowsList = rowsList;
	  };

	  RowControler.prototype.resize = function() {
	    return this.longList.resizeHandler();
	  };

	  RowControler.prototype.goDownHalfBuffer = function(coef) {
	    return this.longList._test.goDownHalfBuffer(coef);
	  };

	  RowControler.prototype.goUpHalfBuffer = function(coef) {
	    return this.longList._test.goUpHalfBuffer(coef);
	  };

	  RowControler.prototype.addRow = function(rankToAdd) {
	    var i, len, results, row, rowsToReRanked;
	    row = this.rowsList.insert(rankToAdd, '');
	    row.data = "row: id_" + row.id;
	    this.longList.addRow(rankToAdd);
	    rowsToReRanked = this.longList.getRowsAfter(rankToAdd);
	    results = [];
	    for (i = 0, len = rowsToReRanked.length; i < len; i++) {
	      row = rowsToReRanked[i];
	      results.push(row.el.lastChild.innerHTML = "current rank: " + row.rank);
	    }
	    return results;
	  };

	  RowControler.prototype.removeRows = function(rankToDelete, nToDelete) {
	    var i, index, len, results, row, rowsToReRanked;
	    index = nToDelete;
	    while (index--) {
	      this.rowsList.remove(rankToDelete);
	    }
	    index = nToDelete;
	    while (index--) {
	      if (this.selectedRows[rankToDelete + index]) {
	        this.selectedRows[rankToDelete + index] = false;
	      }
	    }
	    this.longList.removeRows(rankToDelete, nToDelete);
	    rowsToReRanked = this.longList.getRowsAfter(rankToDelete);
	    results = [];
	    for (i = 0, len = rowsToReRanked.length; i < len; i++) {
	      row = rowsToReRanked[i];
	      results.push(row.el.lastChild.innerHTML = "current rank: " + row.rank);
	  ...
	  RowControler.prototype.removeAllRows = function() {
	    this.rowsList = new LinkedList();
	    this.selectedRows = {};
	    return this.longList.removeAllRows();
	  };

	  RowControler.prototype.addRowAfterSelected = function() {
	    var i, j, len, len1, newRow, rank, row, rowID, rows, rowsList, rowsToReRanked, selectedRows, smallestRank;
	    ...
	    for (i = 0, len = rows.length; i < len; i++) {
	      ...
	      this.longList.addRow(rank + 1);
	      rowsToReRanked = this.longList.getRowsAfter(smallestRank + 1);
	      for (j = 0, len1 = rowsToReRanked.length; j < len1; j++) {
	        row = rowsToReRanked[j];
	        row.el.lastChild.innerHTML = "current rank: " + row.rank;
	      }
	    }
	  };

	  RowControler.prototype.addRowAtRk = function(rank) {
	    var i, len, newRow, row, rowsToReRanked;
	    newRow = this.rowsList.insert(rank, '');
	    newRow.data = "row: id_" + newRow.id;
	    this.longList.addRow(rank);
	    rowsToReRanked = this.longList.getRowsAfter(rank);
	    for (i = 0, len = rowsToReRanked.length; i < len; i++) {
	      row = rowsToReRanked[i];
	      row.el.lastChild.innerHTML = "current rank: " + row.rank;
	    }
	  ...
	  RowControler.prototype.getDataAtRank = function(rank) {
	    return this.rowsList.at(rank).data;
	  };

	  RowControler.prototype.getRowElementAt = function(rank) {
	    return this.longList.getRowElementAt(rank);
	  };

	  RowControler.prototype.deleteSelected = function() {
	    var i, len, rank, row, row$, rowID, rowsList, rowsToReRanked, selectedRows, smallestRank;
	    selectedRows = this.selectedRows;
	    rowsList = this.rowsList;
	    ...
	    for (rowID in selectedRows) {
	      ...
	      this.longList.removeRows(rank, 1);
	    }
	    rowsToReRanked = this.longList.getRowsAfter(smallestRank);
	    for (i = 0, len = rowsToReRanked.length; i < len; i++) {
	      row = rowsToReRanked[i];
	      row.el.lastChild.innerHTML = "current rank: " + row.rank;
	    }
	  ...
	  RowControler.prototype.deleteAll = function() {
	    this.rowsList = new LinkedList();
	    this.longList.removeAllRows();
	    return this.selectedRows = {};
	  };

	  RowControler.prototype.unActivateScrollListener = function() {
	    return this.longList._test.unActivateScrollListener();
	  };

	  RowControler.prototype.activateScrollListener = function() {
	    return this.longList._test.activateScrollListener();
	  };
	  ...
	  RowControler.prototype.testLongListConsistency = function() {
	    var buffer, bufferRow, currentRk, dataRow, dim, domRow$, expectedLength, longList, nElements, ref, rowID, rows$, rowsList, selRow, selectedRows, state, viewport$;
	    longList = this.longList;
	    rowsList = this.rowsList;
	    selectedRows = this.selectedRows;
	    state = longList._test.getState();
	    ref = longList._test.getInternals(), buffer = ref.buffer, rows$ = ref.rows$, viewport$ = ref.viewport$;
	    nElements = rows$.children.length;
	    expectedLength = rowsList.size();
	    expect(expectedLength).to.eql(state.nRows);
	    if (state.nMaxRowsInBufr < expectedLength) {
	      expect(rows$.children.length).to.eql(state.nMaxRowsInBufr);
	    } else {
	      expect(rows$.children.length).to.eql(expectedLength);
	    }
	    dim = rows$.getBoundingClientRect();
	    expect(dim.height).to.eql(expectedLength * state.rowHeight);
	    if (expectedLength === 0) {
	      expect(parseInt(rows$.style.paddingTop)).to.eql(0);
	    } else {
	      expect(parseInt(rows$.style.paddingTop)).to.eql(buffer.firstRk * state.rowHeight);
	    }
	    if (expectedLength === 0) {
	      expect(buffer.firstRk).to.eql(-1);
	      expect(buffer.lastRk).to.eql(-1);
	      expect(buffer.first).to.eql(null);
	      expect(buffer.last).to.eql(null);
	      return;
	    }
	    ...
	    rowID = dataRow.id;
	    while (true) {
	      expect(bufferRow.rank).to.eql(currentRk);
	      expect(parseInt(domRow$.dataset.rank)).to.eql(currentRk);
	      expect(domRow$).to.eql(bufferRow.el);
	      expect(parseInt(domRow$.dataset.rank)).to.eql(bufferRow.rank);
	      expect(dataRow.data).to.eql(domRow$.firstChild.textContent);
	      expect('row: id_' + dataRow.id).to.eql(domRow$.firstChild.textContent);
	      expect('current rank: ' + currentRk).to.eql(domRow$.lastChild.textContent);
	      if (domRow$.classList.contains('selected')) {
	        ...
	        expect(selectedRows[rowID] != null).to.eql(false);
	      }
	      bufferRow = bufferRow.prev;
	      if (bufferRow === buffer.first) {
	        break;
	      }
	      currentRk += 1;
	      dataRow = rowsList.at(currentRk);
	    ...
	    return expect(buffer.lastRk - buffer.firstRk + 1).to.eql(buffer.nRows);
	  };
	  ...
	  RowControler.prototype.testDecorationOfRow = function(rank) {
	    var idDecoration, rkDecoration, row, row$;
	    row$ = this.longList.getRowElementAt(rank);
	    idDecoration = row$.firstChild.textContent;
	    rkDecoration = row$.lastChild.textContent;
	    row = this.rowsList.at(rank);
	    expect(idDecoration).to.eql(row.data);
	    return expect(rkDecoration).to.eql("current rank: " + rank);
	  };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var jade = __webpack_require__(45);
	...
	buf.push("<div class=\"controls\"><button class=\"initBtn\">Initialise rows</button><input type=\"number\" value=\"0\" class=\"nbRowsToIni\"><br><button class=\"goDownBtn\">Go Down Half buffer</button><button class=\"goUpBtn\">Go Up Half buffer</button><br><button class=\"addBtn\">Add  a row after selected</button><br><button class=\"addAtRkBtn\">Add  a row at rank :</button><input type=\"number\" value=\"0\" class=\"insertionRk\"><br><button class=\"deleteBtn\">Delete selected rows</button><br><button class=\"deleteAllBtn\">Delete all rows</button><br><button class=\"unActivateBtn\">un-activate scroll listener</button><button class=\"activateBtn\">activate scroll listener</button><br><button class=\"testLongListBtn\">Test consistency</button><span class=\"consistencyResult\"></span></div><div class=\"long-list-block dynDiv_setLimit\"><div id=\"resizable-container\" class=\"dynDiv_moveDiv dynDiv_hideMove\"><section class=\"resizedContainer longListViewport\"></section><div class=\"dynDiv_resizeDiv_br\"></div></div></div><div class=\"tests\">TESTS<div id=\"mocha\"></div></div>");;return buf.join("");
	}
...
/***/ function(module, exports) {
	...
	module.exports = function(options) {
	  var BUFFER_LIST_LENGTH, LONG_LIST_LENGTH, SHORT_LIST_LENGTH, rowControler;
	  ...
	  describe('1 - Long list tests (100 000 rows) - removeRow()', function() {
	    describe('1.1 - Single deletion of a row before buffer', function() {
	      var initialFirstVisibleRow, stateFinal, stateInitial;
	      stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, nToDelete, rankToDelete;
	      ...
	      it('the longList should have one row less', function() {
	        return expect(stateFinal.nRows).to.eql(LONG_LIST_LENGTH - 1);
	      });
	      it('the bufffer should start one rank earlier', function() {
	        return expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk - 1);
	      });
	      it('the bufffer should be the same length', function() {
	        return expect(stateFinal.buffer.lastRk - stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.lastRk - stateInitial.buffer.firstRk);
	      });
	      it('the first visible row should be the same after deletion (visible rows should remain the same)', function() {
	        var finalFirstVisibleRow;
	        finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	        return expect(finalFirstVisibleRow).to.eql(initialFirstVisibleRow);
	      });
	      it('the height of the list should be 1 row\'s height less', function() {
	        var theoricalHeight;
	        theoricalHeight = stateFinal.rowHeight * (LONG_LIST_LENGTH - 1);
	        return expect(stateFinal.height).to.be.within(theoricalHeight - 3, theoricalHeight + 3);
	      });
	      return it('the final state should be consistant', function() {
	        return rowControler.testLongListConsistency();
	      });
	    });
	    describe('1.2 - Deletion of two rows before the buffer', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	        nRows4Test = LONG_LIST_LENGTH;
	        rowControler.init(nRows4Test);
	        rankToDelete = 2;
	        nToDelete = 2;
	        ...
	        initialFirstVisibleRow = rowControler.getDataAtRank(stateInitial.viewport.firstRk);
	        rowControler.removeRows(rankToDelete, nToDelete);
	        return stateFinal = rowControler._test.getState();
	      });
	      it('the longList should have two rows less', function() {
	        return expect(stateFinal.nRows).to.eql(LONG_LIST_LENGTH - nToDelete);
	      });
	      it('the bufffer should start two ranks earlier', function() {
	        return expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk - nToDelete);
	      });
	      it('the bufffer should be the same length', function() {
	        return expect(stateFinal.buffer.lastRk - stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.lastRk - stateInitial.buffer.firstRk);
	      });
	      ...
	      it('the height of the list should be 2 row\'s height less', function() {
	        var theoricalHeight;
	        theoricalHeight = stateFinal.rowHeight * (LONG_LIST_LENGTH - nToDelete);
	        return expect(stateFinal.height).to.be.within(theoricalHeight - 4, theoricalHeight + 4);
	      });
	      return it('the final state should be consistant', function() {
	        return rowControler.testLongListConsistency();
	      });
	    });
	    describe('1.3 - Single row deletion in the buffer, before viewport', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    describe('1.4 - Single row deletion in the buffer, first row of the buffer.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    describe('1.5 - Single row deletion in the buffer, first row of the viewport.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	      ...
	      it('the first visible row should not be the same after deletion (visible rows should remain the same, except the deleted one...)', function() {
	        var finalFirstVisibleRow;
	        finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	        return expect(finalFirstVisibleRow).not.to.eql(initialFirstVisibleRow);
	      });
	      it('the height of the list should be 1 row\'s height less', function() {
	        var theoricalHeight;
	        theoricalHeight = stateFinal.rowHeight * (LONG_LIST_LENGTH - nToDelete);
	    ...
	    describe('1.6 - Single row deletion in the buffer, last row of the buffer.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    describe('1.7 - Single row deletion in the buffer, but with no row before the buffer, but some after it.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	      ...
	      it('the bufffer should start at the same rank', function() {
	        return expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk);
	      });
	      it('the bufffer should be the same length', function() {
	        return expect(stateFinal.buffer.lastRk - stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.lastRk - stateInitial.buffer.firstRk);
	      });
	    ...
	    describe('1.8 - Deletion of the first row, with no row before the buffer , but some after it.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    describe('1.9 - Deletion of the last row of the buffer, with no row before the buffer, but some after it.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    describe('1.10 - Single row deletion in the buffer, in the viewport.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test, rankToDelete;
	    ...
	    return describe('1.11 - Deletion of all rows.', function() {
	      var initialFirstVisibleRow, nToDelete, stateFinal, stateInitial;
	      nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var nRows4Test;
	      ...
	      it('the longList should have one row less', function() {
	        return expect(stateFinal.nRows).to.eql(0);
	      });
	      it('the bufffer should start at -1', function() {
	        return expect(stateFinal.buffer.firstRk).to.eql(-1);
	      });
	      it('the bufffer should be empty', function() {
	        return expect(stateFinal.buffer.lastRk - stateFinal.buffer.firstRk).to.eql(0);
	      });
	      it('the height of the list should 0', function() {
	        return expect(stateFinal.height).to.eql(0);
	      });
	      return it('the final state should be consistant', function() {
	        return rowControler.testLongListConsistency();
	      });
	  ...
	  describe('2 - Short list tests (less than in the buffer) - removeRows()', function() {
	    describe('2.1 - nRows < nMaxRowsInBufr : Single row deletion in the middle of the buffer.', function() {
	      var initialFirstVisibleRow, nRows4Test, nToDelete, stateFinal, stateInitial;
	      ...
	      it('the longList should have one row less', function() {
	        return expect(stateFinal.nRows).to.eql(nRows4Test - nToDelete);
	      });
	      it('the bufffer should start at rank 0', function() {
	        return expect(stateFinal.buffer.firstRk).to.eql(0);
	      });
	      it('the bufffer should have one row less', function() {
	        return expect(stateFinal.buffer.lastRk - stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.lastRk - stateInitial.buffer.firstRk - 1);
	      });
	      it('the first visible row should be the same after deletion (visible rows should remain the same)', function() {
	        var finalFirstVisibleRow;
	        finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	      ...
	      it('the height of the list should be 1 row\'s height less', function() {
	        return expect(stateFinal.height).to.eql(stateFinal.rowHeight * (nRows4Test - nToDelete));
	      });
	      return it('the final state should be consistant', function() {
	        return rowControler.testLongListConsistency();
	      });
	    });
	    describe('2.2 - nRows < nMaxRowsInBufr : deletion of the first row of the buffer.', function() {
	      var initialFirstVisibleRow, nRows4Test, nToDelete, stateFinal, stateInitial;
	      nRows4Test = nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var rankToDelete;
	    ...
	    return describe('2.3 - nRows < nMaxRowsInBufr : deletion of the last row of the buffer.', function() {
	      var initialFirstVisibleRow, nRows4Test, nToDelete, stateFinal, stateInitial;
	      nRows4Test = nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var rankToDelete;
	      ...
	      it('the initial state should not be dynamic', function() {
	        return expect(stateInitial.isDynamic).to.not.be["true"];
	      });
	      it('the final state should be static', function() {
	        return expect(stateFinal.isDynamic).to.not.be["true"];
	      });
	      it('the longList should have one row less', function() {
	        return expect(stateFinal.nRows).to.eql(nRows4Test - nToDelete);
	      });
	  ...
	  describe('3 - List with on more row than in the buffer (remove one => isDynamic == false)', function() {
	    describe('3.1 -  nRows = nMaxRowsInBufr + 1 : Single row deletion, in the buffer.', function() {
	      var initialFirstVisibleRow, nRows4Test, nToDelete, stateFinal, stateInitial;
	      nRows4Test = nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var rankToDelete;
	      ...
	      it('the initial state should be dynamic', function() {
	        return expect(stateInitial.isDynamic).to.be["true"];
	      });
	      it('the final state should be static', function() {
	        return expect(stateFinal.isDynamic).to.not.be["true"];
	      });
	      ...
	      it('the bufffer should have the same length', function() {
	        var final_nRows;
	        final_nRows = stateFinal.buffer.lastRk - stateFinal.buffer.firstRk + 1;
	        return expect(final_nRows).to.eql(stateInitial.buffer.nRows);
	      });
	      it('the first visible row should be the same after deletion (visible rows should remain the same)', function() {
	        var finalFirstVisibleRow;
	        finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	    ...
	    return describe('3.2 - nMaxRowsInBufr + 1 = nRows : last row deletion, out of the buffer.', function() {
	      var initialFirstVisibleRow, nRows4Test, nToDelete, stateFinal, stateInitial;
	      nRows4Test = nToDelete = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        var rankToDelete;
	  ...
	  describe('4 - Long list tests (100 000 rows) - addRows()', function() {
	    describe('4.1 - there are rows above the buffer', function() {
	      describe('4.1.0 - add a row before the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	          rowControler.init(nRows4Test);
	        ...
	        it('the final state should be dynamic', function() {
	          return expect(stateFinal.isDynamic).to.be["true"];
	        });
	        it('the longList should have one more row', function() {
	          return expect(stateFinal.nRows).to.eql(nRows4Test + 1);
	        });
	        it('the bufffer should start one rank further', function() {
	          return expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk + 1);
	        });
	        it('the bufffer should have the same length', function() {
	          var final_nRows;
	        ...
	        it('the first visible row should be the same after insertion (visible rows should remain the same)', function() {
	          var finalFirstVisibleRow;
	          finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	          return expect(finalFirstVisibleRow).to.eql(initialFirstVisibleRow);
	        });
	        it('the height of the list should have more 1 row', function() {
	          var theoricalHeight;
	      ...
	      describe('4.1.1 - add a row, in the buffer, before the viewport.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	        ...
	        it('the inserted row of the buffer should be decorated', function() {
	          return rowControler.testDecorationOfRow(rankToAdd);
	        });
	        it('the height of the list should have more 1 row', function() {
	          var theoricalHeight;
	          theoricalHeight = stateFinal.rowHeight * (nRows4Test + 1);
	      ...
	      describe('4.1.2 - add a row, in the buffer, on first row of the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	        ...
	        it('the bufffer should start one row further', function() {
	          return expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk + 1);
	        });
	        it('the bufffer should have the same length', function() {
	          var final_nRows;
	      ...
	      describe('4.1.3 - add a row, in the buffer, on second row of the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	      ...
	      describe('4.1.4 - add a row, in the buffer, in viewport.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	      ...
	      describe('4.1.5 - add a row, at the last row of the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	        ...
	        it('the last row of the buffer should be the new one', function() {
	          return rowControler.testDecorationOfRow(stateInitial.buffer.lastRk);
	        });
	        it('the height of the list should have more 1 row', function() {
	          var theoricalHeight;
	          theoricalHeight = stateFinal.rowHeight * (nRows4Test + 1);
	      ...
	      return describe('4.1.6 - add a row after the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	    ...
	    return describe('4.2 - there are no row above the buffer', function() {
	      describe('4.2.1 - add a row, in the buffer, before the viewport.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	      ...
	      describe('4.2.2 - add a row, in the buffer, on first row of the buffer.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	      ...
	      return describe('4.2.4 - add a row, in the buffer, in viewport.', function() {
	        var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	        rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	        before(function() {
	          nRows4Test = LONG_LIST_LENGTH;
	  ...
	  describe('5 - Short list tests (less than in the buffer) - addRows()', function() {
	    describe('5.1 - add a row at rank 0', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = 5;
	      ...
	      it('the initial state should not be dynamic', function() {
	        return expect(stateInitial.isDynamic).to.be["false"];
	      });
	      it('the final state should be dynamic', function() {
	        return expect(stateFinal.isDynamic).to.be["false"];
	      });
	      it('the longList should have one more row', function() {
	        return expect(stateFinal.nRows).to.eql(nRows4Test + 1);
	      });
	      it('the bufffer should still start at 0', function() {
	        expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk);
	        return expect(stateFinal.buffer.firstRk).to.eql(0);
	      });
	      it('the bufffer should have one more row', function() {
	        var final_nRows;
	        final_nRows = stateFinal.buffer.lastRk - stateFinal.buffer.firstRk + 1;
	        return expect(final_nRows).to.eql(stateInitial.buffer.nRows + 1);
	      });
	      it('the first visible row should not be the same after insertion (visible rows should remain the same)', function() {
	        var finalFirstVisibleRow;
	        finalFirstVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk);
	        return expect(finalFirstVisibleRow).to.not.eql(initialFirstVisibleRow);
	      });
	      it('the inserted row of the buffer should be decorated', function() {
	        return rowControler.testDecorationOfRow(rankToAdd);
	      });
	      it('the height of the list should have more 1 row', function() {
	        return expect(stateFinal.height).to.eql(stateFinal.rowHeight * (nRows4Test + 1));
	      });
	      return it('the final state should be consistant', function() {
	        return rowControler.testLongListConsistency();
	      });
	    });
	    describe('5.2 - add a row in the middle', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = 5;
	    ...
	    describe('5.3 - add a row at rank = buffer.lastRk', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = 5;
	    ...
	    return describe('5.4 - add a row at rank = buffer.lastRk + 1', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = 5;
	  ...
	  return describe('6 - Short list tests : with nRows == nMaxRowsInBufr - addRows()', function() {
	    describe('6.1 - add a row at rank 0', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	      ...
	      it('the bufffer should start at 0', function() {
	        expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk);
	        return expect(stateFinal.buffer.firstRk).to.eql(0);
	      });
	      it('the bufffer should be the same length', function() {
	      ...
	      it('the second visible row should be the same after insertion (visible rows should remain the same)', function() {
	        var finalSecondVisibleRow;
	        finalSecondVisibleRow = rowControler.getDataAtRank(stateFinal.viewport.firstRk + 1);
	        return expect(finalSecondVisibleRow).to.eql(initialFirstVisibleRow);
	      });
	      it('the inserted row of the buffer should be in the buffer', function() {
	        var rowElement;
	        rowElement = rowControler.getRowElementAt(rankToAdd);
	        return expect(rowElement).to.not.be.undefined;
	      });
	      it('the height of the list should have more 1 row', function() {
	        return expect(stateFinal.height).to.eql(stateFinal.rowHeight * (nRows4Test + 1));
	      });
	    ...
	    describe('6.2 - add a row in the buffer, before viewport', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = BUFFER_LIST_LENGTH;
	      ...
	      it('the bufffer should still start at 1', function() {
	        expect(stateFinal.buffer.firstRk).to.eql(stateInitial.buffer.firstRk + 1);
	        return expect(stateFinal.buffer.firstRk).to.eql(1);
	      });
	      it('the bufffer should have same length', function() {
	        var final_nRows;
	        final_nRows = stateFinal.buffer.lastRk - stateFinal.buffer.firstRk + 1;
	        return expect(final_nRows).to.eql(stateInitial.buffer.nRows);
	      });
	    ...
	    describe('6.3 - add a row in the viewport', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = BUFFER_LIST_LENGTH;
	      ...
	      it('the inserted row of the buffer should be in the buffer', function() {
	        var rowElement;
	        rowElement = rowControler.getRowElementAt(rankToAdd);
	        return expect(typeof rowElement).to.eql('object');
	      });
	      it('the height of the list should have more 1 row', function() {
	        return expect(stateFinal.height).to.eql(stateFinal.rowHeight * (nRows4Test + 1));
	      });
	    ...
	    describe('6.4 - add a row at rank = buffer.lastRk', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = BUFFER_LIST_LENGTH;
	    ...
	    return describe('6.4 - add a row at rank = buffer.lastRk + 1', function() {
	      var initialFirstVisibleRow, nRows4Test, nToAdd, rankToAdd, stateFinal, stateInitial;
	      rankToAdd = nRows4Test = nToAdd = stateFinal = stateInitial = initialFirstVisibleRow = null;
	      before(function() {
	        nRows4Test = BUFFER_LIST_LENGTH;
	      ...
	      it('the inserted row of the buffer should be outside the buffer', function() {
	        var rowElement;
	        rowElement = rowControler.getRowElementAt(rankToAdd);
	        return expect(rowElement).to.be.undefined;
	      });
	      it('the height of the list should have more 1 row', function() {
	        return expect(stateFinal.height).to.eql(stateFinal.rowHeight * (nRows4Test + 1));
	      });
