...
# request = require("browser-request") # ne peut pas fonctionner, lastpass refuse les requêtes CORS.
request = require("./request-browserify-4Cozy.coffee")

_request_iteration_count = (username, cb) ->
    options =
    ...
    request options, (err, resp, body) ->
        console.log '_request_iteration_count response body', body, err
        cb(body)
...
_request_session = (username, password, key_iteration_count, multifactor_password, cb) ->
    console.log '_request_session(', username, password, key_iteration_count, multifactor_password, ')'
    options =
...
Fetcher.login = (username, password, multifactor_password, cb) ->
    ...
    _request_iteration_count username, (key_iteration_count)->
        _request_session(username, password, key_iteration_count, multifactor_password, cb)
    # _request_session(username, password, 5000, multifactor_password, cb)
